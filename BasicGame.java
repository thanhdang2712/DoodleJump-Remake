...
public class BasicGame extends AbstractGame {
    ...
    private static final int generateEntities = 10;
    ...
    private boolean isWithinBounds(Entity e) {
        return (e.getX() >= 0 && e.getY() >= 0 
        && e.getX() + e.getWidth() <= this.getWindowWidth()
        && e.getY() + e.getHeight() <= this.getWindowHeight());
    }
    ...
    private void handleMovementKeys(int key) {
        ...
        if (key == UP_KEY) {
            yCoord = player.getY() - player.getMovementSpeed();
